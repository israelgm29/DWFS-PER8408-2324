...
function ejemplo() {
  ...
  console.log(pixeles);
  handler.savePixels(pixeles, outputPath, row, col);
}
...
function redConverter() {
  ...
  //Aqui tu codigo
  let row = pixels.length;
  let col = pixels[0].length;
  for (let i = 0; i < row; i++) {
    for (let j = 0; j < col; j++) {
      pixels[i][j][1] = 0;
      pixels[i][j][2] = 0;
 ...
 *
 * Una forma de conseguirlo es quitar los valores de las row y col pares.
 * Otra forma es crear la imagen de nuevo unicamente con los valores de las row y col pares.
 */
function scaleDown() {
...
function merge(alphaFirst, alphaSecond) {
  ...
  //Aqui tu codigo
  let row = catPixels.length;
  let col = catPixels[0].length;
  for (let i = 0; i < row; i++) {
    let nuevaFila = [];
    for (let j = 0; j < col; j++) {
      let pixel = [0, 0, 0];
      pixel[0] = (catPixels[i][j][0] * alphaFirst + dogPixels[i][j][0] * alphaSecond) % 255;
      pixel[1] = (catPixels[i][j][1] * alphaFirst + dogPixels[i][j][1] * alphaSecond) % 255;
      pixel[2] = (catPixels[i][j][2] * alphaFirst + dogPixels[i][j][2] * alphaSecond) % 255;
